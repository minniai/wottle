...
- **Dictionary & Words:** Words are validated against the **full BÍN inflected word list** (~3.7M forms, `data/wordlists/word_list_is.txt`; the game loads the ~1.16M that fit the board, `word_list_10_is.txt`; curated by `word_list_is_exclusions.txt`; rules §3.3). Words are recognized case-insensitively, but language-specific characters are treated as distinct letters (e.g. **Ð** vs **D**). The architecture is designed to support plugging in different dictionaries for other languages in future releases.
...
- **Icelandic MVP:** the full inflected BÍN word list (~3.74M forms, `data/wordlists/word_list_is.txt`, loaded as `word_list_10_is.txt` with the words longer than the board stripped), minus a small curated exclusions file. BÍN entries and nothing else (rules §3.3).
