...
- **Every match begins at a table** (spec 069). A player is seated at creation by their own press (accepting a challenge or rematch, or a send that crossed the other player's) or when their tab was visible and used in the last 30s; anyone else has **20s from the match's creation** to press `ready ▸`. The server holds the letters until both are seated. The seat that fills the table writes the board and sets `matches.started_at` to that moment + 4.5s; the room counts 3·2·1 while the letters land, and the match starts at `started_at`.
- A match has **one clock**, shared by both players: **5:00 (300 000 ms)** from the moment the match starts (`matches.started_at`, `matches.deadline_at = started_at + 5:00`). It **never pauses** — not for a reveal, not for a disconnection — and it never stops early except by the match ending. The design draws it once, in the scoreboard's clock row (§12).
...
- **The match ends** when both players have ten resolved moves, or when the deadline has passed and every move received before it has resolved. First, every move a player has not made is scored as a miss (§5.6); then it is decided in this order (`lib/match/resultCalculator.ts`; amended 2026-09-21):
  ...
  3. still tied → **draw**.
  Running out of time is not a loss by itself. The recorded reason says who was short: `moves_complete` (both had ten), `incomplete` (one did not), `both_incomplete` (neither did). A match ended early (the player with ten moves ends it once the other has been gone past the 90s window) is recorded as `ended_early` and settled exactly like `incomplete`: the absent player's unplayed moves are penalised and the normal rules decide (spec 071). Every match that starts is rated. A table where both players do not sit down within 20s, or that a player leaves before go, is void: not a match, no rating, no history, no rematch.
...
- **The void** (spec 069). A void match is stored as `completed` with `ended_reason = 'void'`, its reason (`not_seated` or `left`) and the player who voided it; it has no winner and appears in no history, record, statistic, recent-matches list or rematch offer. A seated queue player is requeued at the front, keeping their original join time; a player who did not sit down has their search stopped. Two `left` voids by one player within 10 minutes bar them from searching and sending challenges for 5 minutes; accepting stays open.
- Resigning ends the match at once as a forfeit (`forfeit`, the other player wins). A match that has not started cannot be resigned; leaving the table voids it. Nothing else resigns: signing out never ends a match, and is refused while one is live (spec 067).
...
The word, normalized to NFC and lowercased, must be present in the active language's dictionary. For Icelandic, this is the full inflected BÍN word list (~3.74M entries), stored in `data/wordlists/word_list_is.txt`. The game loads the **board wordlist** built from it, `word_list_<min>_<max>_is.txt` (`word_list_3_10_is.txt`, ~1.16M entries): a word shorter than the minimum word length never scores and a word longer than the board can never be spelled on it, so `pnpm wordlists:build` strips both, and the loader throws if the board wordlist for the current lengths is missing. **The dictionary accepts BÍN entries and nothing else** — there is no additions mechanism; if a real word is missing (e.g. the *kóla* paradigm, Linear O-70), the fix is regenerating the wordlist from BÍN upstream, never an in-repo addition. A small exclusions file, `word_list_is_exclusions.txt`, is subtracted at load time to remove BÍN entries rejected as playable words (e.g. *sýs*, Linear O-81); it accepts one word per line with `#` comments, NFC-normalized and lowercased on load. Accented and unaccented vowels are distinct letters: *ílæti* is a valid BÍN word while *ilæti*/*itæli* are not, and lookups never conflate them (O-69).
...
- A **miss** is a resolved move that scores no word. A miss costs **−5**, but never more than the player has: at 4 points it costs −4, at 0 it costs nothing (`missPenaltyFor`, `lib/scoring/missPenalty.ts`). The resolver prices it against the mover's total at claim time, which is exact because moves resolve one at a time (§7).
- A **refused** move (`frozen` / `moved`, §2) is not a move: it costs nothing and does not count.
- At the deadline, every move a player has not made is a miss too: a player with three unplayed moves loses a further 15, or their whole total if it is less. These penalties are written into the totals in the same step that completes the match, so they apply once (`app/actions/match/completeMatch.ts`).
- A resignation or a win by disconnect names its winner and applies no timeout penalty.
- A total never goes below zero (2026-09-22). Matches played on 2026-09-21–22 before the floor may hold negative totals; they are drawn with a real minus.
...
| Rule | Rendering |
| --- | --- |
| A scored word (§3, §6) | A **band** along its tiles on the field: a 14% tint of the scorer's seat colour (teal = you, terracotta = the opponent), square ends aligned to the cell grid, with the letters inside drawn in the scorer's colour. |
...
| Frozen tile (§6) | The letter sits inside a settled band and cannot be picked; tapping it shakes the letter 300ms in its own colour and the ledger's live row names the word and its owner for two seconds: `frozen · GILT · Kári · pick another` (`frozen · Kári M2 · pick another` when the word is not known). |
...
| A move refused (§2) | The two letters return; the live row reads `frozen · Kári froze it · pick another` or `moved · Kári moved it · pick another` for two seconds (Icelandic `Kári frysti stafinn · veldu annan`, `Kári færði stafinn · veldu annan`); the move count is unchanged, and a refused move never becomes the last move. |
| The opponent's move on your field (§2) | Their letters exchange and their bands draw at 30% while you pick; nothing locks. A pick on a letter they exchanged or froze clears, and the live row's line 2 reads `pick cleared · Kári moved that letter` for two seconds (it is not a notice). Each opponent move is announced once to assistive technology as it arrives (`Kári SKÓ +11 · 5 of 10`). |
| The last move (§2) | A **letter state**: a 2px bar in the mover's seat colour along the bottom inner edge of the two cells of each player's most recent resolved swap, at most two cells per seat, dropped when the letter freezes. A refused move is never the last move; after a reload onto one there is no tick until that player moves again. The cell's accessible name ends `, Kári's last move`. |
| Clock (§2a) | Once, as the **scoreboard's** first row above the field (spec 068): a two-line label (`match clock` over the pace `≈27s a move` while the move is yours), a track of ten 30s blocks of six 5s ticks (one tick empties every 5s from the right), and the numeral `3:12`. Urgency is weight only and nothing blinks: under 1:00 the row takes the tint, ink ticks and a heavier numeral (`under a minute`); the last 15 seconds read `last 12s`; at 0:00 the track is empty and the label reads `time`. Before go the scoreboard is in its `starting` phase: 4.5s after the table fills, the clock row counts `starts in 3 · 2 · 1` from the server's `started_at` and loads left to right while the letters land, both rows read `ready`, and the live row reads `move 1` over `pick when the clock starts`; go is `started_at`; at match over it holds what was left (`4:52 of 5:00`). The clock is drawn nowhere else, and it steps once a second under reduced motion too. |
| The table (§2a) | Every match opens at a table: the scoreboard names both players (`on the way` / `ready`), the clock row reads `match clock` over `starts when both sit` with a full track at `5:00` that does not run, and the field is the empty ruled frame at 32% under the **ready slip**: `opponent found · 0:14` counting down the 20s, the opponent's name, the facts, the stakes (`win +8 · draw 0 · loss −8`), both seat lines, `ready ▸` (ignored for 500ms, never focused) or `you are seated`, `leave` on its own row, and a drain along the bottom edge. A player seated by their own press, or whose tab was visible and used in the last 30s, is seated at once. No letters reach either client until both are seated; the start is set 4.5s after the second seat. A table not filled in 20s, or left (`leave`, or Back) before go, is **void**: the slip rewrites to `no match` over `Kári did not sit down` · `Kári left the table` · `You did not sit down in time` and `nothing was rated`, then by origin `you are back in the queue` with `cancel ▸` (a seated queue player is requeued at the front), `challenge again ▸` · `lobby`, or `result ▸` · `lobby`. The player who did not sit down finds `you did not sit down · your search stopped` in the lobby. After two `left` voids in 10 minutes the lobby's find action reads `find again in 4:12` for 5 minutes. |
| Moves and progression (§2) | In the ledger: ten rows indexed by move number (the ledger names no move of the viewer's; the move rail went on 21 September 2026) — your Nth move in your column, theirs in theirs — with your next open move as the tinted **live row** whose first line names the beat (`move 4 · your move`, `move 4 · scoring`, `move 4 scored`, `10 of 10 played`, `time · scoring`). On the scoreboard, each player's row (the opponent's, then yours nearest the board) draws their ten moves on the same track as the clock, directly under its blocks, so time left and moves left compare down a column: a resolved move empties the rightmost, a move in flight shows at 30%, a reconnecting or gone player's (and your own while you are offline) are outlined; the sub-line carries the count (`1310 · you · move 4 of 10`, `6 of 10 · playing`, `6 of 10 · scoring`, `10 of 10 · done`) and yours reads `move 8 · behind pace` when your moves left exceed the clock's blocks left by one or more. |
...
| Scoring (§5) | Written into the row as each band lands, the move's points beside the move number, which sits in a ruled spine between the two players' columns (`BORÐ 22 │ 1 │ 15 GILT`, 2026-09-21), match totals counting up on the scoreboard. A resolved move with no word writes its penalty labelled (`no word −5`, or less near zero, §5.6), and the live row holds the **missed beat**: `move 4 · no word` over `−5 · move 5 opens`, or `−3 · a total never falls below 0` when the floor takes part of it. Under 1:00, with a move yours and nothing picked, line 2 states the stakes of the timeout penalty: `3 moves left · −15 if unplayed`, or `nothing to lose` when the penalty is 0; at a timed-out end each unplayed row writes its penalty (`−5 not played`). Only the number of points lost is crimson (`--err`); its label stays muted, and a penalty floored to 0 is muted, not crimson. Each row's penalty is priced against the running total of the moves before it. The final ledger closes on a totals row across the spine. A negative total reads with a real minus (`−12`), and a score line with one reads `−4 to −12`. |
| Repeated word (§3.7) | Scored and drawn like any other; nothing marks it. |
| Reconnection window | The disconnected player's moves on the scoreboard are outlined; their sub-line counts `reconnecting · 0:42 left` during the 90s window and `8 of 10 · gone for 2:04` after it (never a frozen `0:00 left`). The clock keeps running. When the window is spent and the viewer has ten moves, a **slip** offers `Kári is gone` (the headline takes focus) · `the normal rules decide it` · `end the match ▸` (ignored for 500ms after it appears) · `keep waiting ▸`; otherwise nothing is offered. `keep waiting ▸` puts the slip away for the rest of the match and the offer moves to the live row's line 2, `Kári is gone · end the match ▸`. |
| Your own connection | Two failed safety polls in a row (about 4s) or the browser's `offline` event mark you offline: your scoreboard row reads `offline · reconnecting` with your moves outlined, line 2 says the same, the field's frame is ink and the field takes no pick. The clock keeps running. The first good poll brings you back without a page load and clears your disconnect on the server; line 2 then reads `back · away 0:34 · the clock ran on` for four seconds. |
| Match over (§2a, §5.5) | A slip over the field, 600ms after the final reveal has held (at once when the result is opened later). Its headline takes focus and is announced once; every action ignores activation for 500ms after it appears or changes meaning. `Kári wins` in the winner's ink (`Draw` in ink), both totals, then **one detail line that says why the match ended, once**: `by 46 points · 10 words to 8 · territory 27–21` (both finished), `Kári played 8 of 10 · by 12 points`, `neither finished · by 12 points`, `Kári resigned · 3:12`, `ended early · Kári was gone`; a phone shows its first two clauses. Then `your best word · BORÐA 29`, both rating lines, and two action rows: `rematch ▸` (primary, not focused) · `new opponent ▸`, then `review the match ▸` · `lobby`. The label counts the match: `match over · 4:52`; the tab reads `Kári wins · Wottle`. Esc or `review the match ▸` lifts the slip, `result ▸` restores it, and the game never raises it again. **Rematch** is offered only while both players have a tab on this match (hidden counts) within 2:00 of its end, and once per match: a request lasts 30s with a drain and `cancel ▸`, and replaces action row 1 on both slips (`rematch sent · 0:24`; `Kári asks for a rematch · 0:24` with `accept ▸` · `decline`). With the slip lifted or in review, a request is the ledger's first line instead and never raises the slip (the cue, `(1) Kári asks for a rematch · Wottle`, the favicon). A decline or 30s without an answer ends it for both and starts the pair's 60s challenge cooldown; crossed presses start one match at once; the new table replaces the result in history. Without a rematch the primary is `new opponent ▸`, with `challenge again ▸` (`again in 0:52` during the cooldown) when the opponent is here. Reactions are phase 2. The ledger keeps the verdict beneath the slip. |
...
| Resigning | A slip: `Resign the match?` with the viewer's move count and the clock (`move 4 of 10 · 3:12 left`) and `Kári wins · your rating moves as a loss`; `keep playing ▸` is the primary and has focus, `yes, resign ▸` is secondary on its own line, and Escape keeps playing. The clock keeps running. |
| Review (spec 071) | `?review=n` on the match page, the same field: every received move is a step, in the order the server received it; a refused move is a step that changes nothing (`refused · frozen`); when unplayed moves were penalised (0:00, or an early end) a closing step applies them (`time · −15 not played`), so the last step equals the result. At step k the field shows the board after it, with every band scored up to k (step k's at 30%, earlier ones at 14%) and step k's swap ticked; the scoreboard's clock row is the scrubber (`step 7 of 20`, the clock when the move was received, a bar; a slider owning ←/→, Home/End and Space), and the player rows show each total and moves at k (`3 of 10 at step 7`). The ledger's cells jump to their step (one grid, one tab stop; those not yet reached are muted and named so), the step's row carries the cursor line (`move 3 · Birna · LEK · ÆSKU +33` over `froze 6 · Birna leads 51–18`), and `first · back · play ▸ · next · last` step it (play: one step a second, stopped by any input). Stepping forward exchanges the step's letters; back or a jump is instant. Entering review adds one history entry and each step replaces it: Back from review is the result, Back again the lobby. `/summary` opens the last step. A finished match's review is readable by anyone, signed in or not: no slip, no rematch, the line `this match is over · Birna – Kári`, and `enter the lobby ▸` for a visitor. Best-here hints are phase 2. |
| An invite link (spec 072) | `invite a friend ▸` in the lobby (the primary of an empty lobby) makes a single-use link that works for 10 minutes and copies it; the line slot reads `Link copied · valid 9:58` with `copy again ▸` · `cancel link ▸` and a 10:00 drain. The link is the sender's one outgoing challenge: sending a challenge, searching, accepting anything, switching lobby or signing out cancels it. Opening the link changes nothing (a chat app's preview cannot use it): signed out it is the door with the call `Kári challenges you` over `1265 · English words · link valid 9:12` and `accept ▸`, which signs the friend in and seats them; signed in it is the lobby with `Kári invites you by link · accept ▸` in the slot; the sender's own link reads `this is your link · copy ▸`. A used, cancelled, expired or unknown link reads `this link has expired`. Two accepts at once make one match. |
| The link table (spec 072) | A link's table waits for its sender until the link would have expired, not 20s; the friend's slip reads `the table waits · 9:12` while the sender is away. The friend may leave it: that is the sender not sitting down (`not_seated`), and it never counts toward the friend's table-leave cooldown. |
| A profile (spec 072) | A page, not a room: the name and the rating in this page's language (in the owner's seat colour: yours `--you`, another player's `--opp`, a visitor sees `--you`), `@handle · playing since March 2026 · 35 matches`, `rating · English · peak 1216 · +16 this week`, a 30-day rating chart (a flat line with `no matches in the last 30 days`), the last ten, the record (won · lost · drawn · win rate = won ÷ matches) and the three best words drawn as bands on cells. Another player's profile adds one presence word (`here now`, `in a match · 6 of 10`, `away`, `in the Icelandic lobby`, `not here`), never a time, and `challenge ▸` as its one primary with the viewer's stakes beneath (`english words · win +7 · draw −1 · loss −9`). Void and abandoned matches never count. |
| Every match that starts is rated (§2a) | No caption or state says otherwise; a rating line reads `rating pending` until the row is written. A void table is not a match: it has no rating line, no verdict and no row in any history. |
| Which board the room shows | `matches.board`, the live board written by every resolved move; a finished match keeps the board its last resolved move left. The starting board is generated once, when the table fills (§2a; spec 049's regeneration fault cannot recur: there is no round to look up). |
