...
When a player makes a move, the swapped tiles are shown as before with animation, but they should only stay on screen for a few seconds or until the scored words are shown, such that if one of the swapped letters is not part of a scored word it should simply disappear from the board. If neither of the swapped tiles are part of a scored word both just dissappear when.

When scored tiles are shown for the current round should be shown distinctivly separate from previously scored tiles, so that the players see what words are scored in this round until the round is completed.
