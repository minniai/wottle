...
function App() {
  ...
  // Apply theme class to <body> (not <html>) so inline CSS vars on <html>
  // don't out-specificity the theme class rules
  React.useEffect(() => {
    const body = document.body;
    if (tweaks.theme === "dark") body.classList.add("theme-dark");
    else body.classList.remove("theme-dark");
  }, [tweaks.theme]);

  // Apply dynamic CSS vars — on <body> so they live alongside the theme class
  // and theme-specific tint/deep values win
  React.useEffect(() => {
    const r = document.body;
    const isDark = tweaks.theme === "dark";
    if (isDark) {
      r.style.setProperty("--p1", `oklch(0.74 0.15 ${tweaks.p1Hue})`);
      r.style.setProperty("--p1-tint", `oklch(0.32 0.08 ${tweaks.p1Hue})`);
      r.style.setProperty("--p1-deep", `oklch(0.86 0.13 ${tweaks.p1Hue})`);
      r.style.setProperty("--p2", `oklch(0.66 0.09 ${tweaks.p2Hue})`);
      r.style.setProperty("--p2-tint", `oklch(0.30 0.05 ${tweaks.p2Hue})`);
      r.style.setProperty("--p2-deep", `oklch(0.80 0.08 ${tweaks.p2Hue})`);
    } else {
  ...
  return (
    <div className={`app-shell tiles-${tweaks.tileStyle} lobby-${tweaks.lobbyVariant} ${tweaks.theme === "dark" ? "theme-dark" : ""}`} data-screen-label={screen} data-lang={lang}>
      {/* Top bar */}
      ...
      {(tweaksOpen || editModeOn) && (
        <div className="tweaks">
          ...
          <div className="tweak-row">
            <label>Theme</label>
            <select value={tweaks.theme || "light"} onChange={e => updateTweak("theme", e.target.value)}>
              <option value="light">Light · warm paper</option>
              <option value="dark">Dark · dusk reading room</option>
            </select>
          </div>
        </div>
      )}
