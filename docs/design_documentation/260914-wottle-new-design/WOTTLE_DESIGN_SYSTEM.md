...
4. **One clock, two counts.** The match has one 5:00 clock, drawn once in the ledger caption; each player's ten moves are drawn as a lane in their bar, so "who is ahead on moves" is a glance and the clock is never drawn twice (amended 21 September 2026, spec 050).
...
Motion is a state change, not a performance. Durations: ring changes 120ms; letter exchange 150ms (preview **and** commit — the two letters travel to each other's places, they never teleport); band draw 400ms per word, staggered 120ms; total count-up 400ms; found-opponent name write 200ms; lane blink 1Hz colour-only; setting-field letters ~100ms apart. Easing `cubic-bezier(0.2,0,0.2,1)`. Geometry does not animate except the two listed (preview exchange, picked scale); the turn frame is an outline. The slip (§5.9) fades in and out with the field over 150ms and is the only thing ever placed over the field. The move hold is 600ms (spec 050; it was 1200ms as the settle hold) and is a reading pause, not motion: it stays under `prefers-reduced-motion`, where everything else is 0ms and end-state only and the lane holds solid instead of blinking.
...
| --- | --- | --- | --- |
| Set | untouched; the turn frame in `--you` | your lane at your count; `· move 4 of 10` in `--you` | your next row opens: `move 4 · your move` over `pick a letter`; the rail's next cell is framed |
...
- Lanes: `role="progressbar"`, `aria-valuemin=0 aria-valuemax=10 aria-valuenow=<moves>`, `aria-valuetext="4 of 10 moves played"`. The caption clock is a `role="timer"` named `match clock`, updated once a second, `aria-live="off"` (the live row announces the beats).
- Live row: `aria-live="polite"` (line 1 announces each beat once); verdict: `aria-live="assertive"` once. The move rail is one `role="img"` named `move 4 of 10`; its cells are hidden.
