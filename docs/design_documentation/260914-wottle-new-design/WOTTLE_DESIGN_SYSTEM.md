...
Motion is a state change, not a performance. Durations: ring changes 120ms; letter exchange 150ms (on commit — the two letters travel to each other's places, they never teleport); band draw 400ms per word, staggered 120ms; total count-up 400ms; found-opponent name write 200ms; ledger clock flash 1Hz in steps over the last 15 seconds; setting-field letters ~100ms apart. Easing `cubic-bezier(0.2,0,0.2,1)`. Geometry does not animate except the two listed (commit exchange, picked scale); the turn frame is an outline. The slip (§5.9) fades in and out with the field over 150ms and is the only thing ever placed over the field. The move hold is 600ms (spec 050; it was 1200ms as the settle hold) and is a reading pause, not motion: it stays under `prefers-reduced-motion`, where everything else is 0ms and end-state only and the ledger clock's inverted face holds instead of flashing.
