...
const rows = buildLedgerRows({ currentRound: 4, completed: false, words: [], playerAId: "a", viewerSlot: "player_a", live: { kind: "idle" } });
const base: LedgerModel = { caption: "round 4 of 10", round: 4, completed: false, rows, territory: EMPTY_TERRITORY, hint: "" };
...
describe("Ledger rail (spec 048 US3)", () => {
  it("match and final render the rail directly after the caption", () => {
    ...
    expect(caption.nextElementSibling).toBe(screen.getByTestId("round-rail"));
    expect(screen.getByTestId("round-rail")).toHaveAttribute("aria-label", "round 4 of 10");
  });
  it("final fills every cell", () => {
    render(<Ledger variant="final" model={{ ...base, round: 10, completed: true }} viewerName="B" opponentName="K" onAction={() => {}} />);
  ...
  it("the queue shows ten future cells; the lobby shows none", () => {
    const { unmount } = render(<Ledger variant="queue" model={{ caption: "10 rounds", rows: [], territory: EMPTY_TERRITORY, hint: "" }} viewerName="B" opponentName={null} body={<span />} onAction={() => {}} />);
    expect(screen.getByTestId("round-rail").querySelectorAll('[data-state="future"]')).toHaveLength(10);
    unmount();
    ...
    expect(screen.queryByTestId("round-rail")).toBeNull();
  });
