...
const bands: WordBand[] = [
  { id: "a", seat: "you", cells: [{ x: 1, y: 2 }, { x: 2, y: 2 }], wordCells: [{ x: 1, y: 2 }, { x: 2, y: 2 }], direction: "ltr", strength: "settled", move: 1, word: "ab" },
  { id: "b", seat: "opp", cells: [{ x: 5, y: 4 }, { x: 5, y: 3 }], wordCells: [{ x: 5, y: 4 }, { x: 5, y: 3 }], direction: "btt", strength: "live", move: 2, word: "cd" },
];

describe("FieldBands (design system §5.2)", () => {
  it("renders a rect and a chevron per band with seat, direction and move", () => {
    render(<FieldBands bands={bands} highlightMove={null} />);
    ...
    expect(els[1]).toHaveAttribute("data-move", "2");
  ...
  it("draws the chevron at the design's 1.5px, not a hairline (spec 045 A3)", () => {
    ...
    const { container } = render(<FieldBands bands={bands} highlightMove={null} />);
  ...
  it("dims bands of other moves while a ledger row is hovered", () => {
    render(<FieldBands bands={bands} highlightMove={2} />);
    const els = screen.getAllByTestId("field-band");
  ...
  it("drawnCount limits how many bands are drawn (reveal)", () => {
    render(<FieldBands bands={bands} highlightMove={null} drawnCount={1} />);
