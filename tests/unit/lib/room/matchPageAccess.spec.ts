...
const BASE: MatchPageFacts = {
  ...
  signedIn: true,
  participant: true,
  match: { state: "completed", endedReason: "moves_complete", language: "en" },
};
...
describe("matchPageAccess (spec 071 FR-041, FR-042, T060, T064)", () => {
  it("renders the match for its players", () => {
    ...
    expect(at({ match: { ...BASE.match, state: "in_progress", endedReason: null } })).toEqual({ kind: "render", readOnly: false });
  });
  ...
  it("sends a stranger away from a live match, and a visitor to the door with the way back", () => {
    const live = { ...BASE.match, state: "in_progress" as const, endedReason: null };
  ...
  it("has nothing to read for a void table", () => {
    const voided = { ...BASE.match, endedReason: "void" as const };
  ...
  it("opens a match in its own language, keeping the query", () => {
    ...
    expect(at({ locale: "en", match: { ...BASE.match, language: "is" } })).toEqual({ kind: "redirect", to: "/match/m1?review=5" });
  });
