...
- **The live row says the beat and the next step** (`lib/room/moveState.ts` `deriveMoveState` → `liveLinesFor` → `{ line1, line2 }`, spec 050; `roundState.ts` until P1): line 1 is the move's beat in the board face (`move 4 · your move`, `move 4 · scoring`, `move 4 scored`, `10 of 10 played`, `time · scoring`); line 2 is the field's instruction while a move is yours to make (`pick a letter`, `picking · T (2) · tap a second letter`, `24 · hestur · tap again to play · esc cancels`, `frozen · Kári just froze it · pick another` for two seconds) or the beat's fact otherwise (`you +13 · move 5 opens`, `waiting for Kári · 8 of 10 · 1:12 left`). After your own reveal the scored row holds 600ms (the move hold) before the next opens, and the field takes no pick meanwhile; the opponent's reveal never locks your field, and a pick on a letter they exchanged or froze clears with a notice.
- **One clock, two counts (spec 050):** the match clock is drawn once, in the ledger caption (`move 4 of 10 · 3:12`; weight 600 and colour-only blink under 1:00); it never appears in a bar. Each bar's lane is that player's moves 0–10 in the seat colour and its sub-line carries the count (`move 4 of 10` in `--you` while a move is yours to make, `move 4 of 10 · scoring`, `10 of 10 · done`; theirs `6 of 10 · playing` / `· scoring` / `10 of 10 · done`). The field is outlined 3px in `--you` only while a move is yours to make. The **move rail** (ten cells under the caption) counts the viewer's moves; ledger rows are indexed by move number, your Nth move in your column and theirs in theirs, and the live row is your next open move. Nothing is ever pinned: a committed move resolves at once.
- The hint line is for match-level lines only and hides when empty. The ledger is the height of the stack, never stretched to the viewport; every row is one grid item that owns its `--rule`. A scored letter and its numeral take the colour of the player who froze it first (`ownerSeatOf`); a crossing keeps the earlier owner and the later word's band covers only the letters it froze (spec 049). No cell is ever `shared`.
...
- `/lib` - Business logic and utilities
  ...
  - `/lib/match` - **Match orchestration**: spec 050 → `moveResolver.ts` (claim → resolve one → finish), `matchSettlement.ts`, `movePublisher.ts`, `resultCalculator.ts`, `stateLoader.ts`, `wordScoreRow.ts`, `previewSchemas.ts` (until P1: `roundEngine.ts`, `stateMachine.ts`, `conflictResolver.ts`, `instantScoring.ts`, `recoverStuckRound.ts`)
  - `/lib/room` - **Room state and pure UI logic**: `roomStore.ts` (zustand: phase lobby | queue | found | match | final, viewer, board, match, connection), `fieldInteraction.ts` (idle → picked → preview → committed reducer), `moveState.ts` (the six move beats; `roundState.ts` until P1), `revealSequence.ts` (reveal planner, `MOVE_HOLD_MS`), `ledgerRows.ts`, `moveRail.ts`, `bandGeometry.ts`, `clock.ts`, `notices.ts`, `displayBoard.ts`, `useMatchmaking.ts`, `useRematchNegotiation.ts`
...
#### 4. Move Resolution (`/lib/match/moveResolver.ts`, spec 050; `roundEngine.ts` until P1)
...
3. Resolve  - resolveOne (pure): refuse `frozen` / `moved`, else applySwap → scan → cross-validate
              → score → freeze (>= 24 unfrozen); no duplicate suppression
...
6. Settle   - both counts = 10, or deadline passed and the queue drained → completeMatchInternal
              (CAS on state) → determineMatchWinner: moves first, then score, tiles, draw → Elo once
```
...
- `Match`: PlayerIdentity, MatchState (players, clock, resolvedSeq), MoveResolution, MatchPhase, FrozenTile, FrozenTileMap, WordScoreBreakdown (spec 050; `RoundSummary`, `PartialRoundSummary`, `SubmissionRecord`, `TimerState` until P1)
...
- **170 unit/contract test files, 1425 tests passing** (2 intentionally skipped), zero failures (measured 2026-09-20 on `049-scored-letter-integrity`). `pnpm test:visual` passes at three viewports over 21 fixture phases.
- CI splits Playwright by tag: the one spec tagged `@two-player-playtest` (`rounds-flow`, a whole match; `moves-flow` after spec 050 P2) runs on the `playtest-firefox` project with `--workers=1`; every other room spec runs on `chromium`. On failure the job uploads `playwright-results-<suite>` (traces + `error-context.md` page snapshots). Locally, run two-player spec files one at a time to avoid Realtime contention; sign in through `loginViaSlip` (`tests/integration/ui/helpers/matchmaking.ts`).
...
| Move Resolution  | Spec 050   | Receipt order, claim/finish CAS, refusal not consumed; rounds until P1 lands     |
...
| Match Clock      | Spec 050   | One 5:00 clock, deadline decided at receipt; per-player timers until P1 lands    |
...
| Move legibility  | Spec 050   | Move state, turn frame, bar counts, 600ms hold, move rail, caption clock         |
...
- **Move**: One player's swap with its receipt sequence, status, per-player sequence, snapshots and delta (match_moves table, spec 050; `rounds` + `move_submissions` until P1)
