...
**The result, rematch and review (spec `specs/071-result-rematch-review/spec.md`, 2026-09-24, branch `071-result-rematch-review`)**: the match ends properly and can be read back.
- **The result.** The match-over slip lands 600ms after the last reveal (at once when opened later), focuses its headline, and guards every action for 500ms (`useActivationGuard`). One detail line says why it ended, once (`resultDetail`: both finished, `incomplete`, `both_incomplete`, `forfeit` with the clock, and a new `ended_early`, which `claimWin` records and which settles like `incomplete`); a phone shows two clauses. Two action rows; the best word; the tab reads `Birna wins · Wottle`. At completion the live guard steps back past every guard entry (the table's too), so one Back from the result reaches the lobby.
- **Rematch.** One locked `request_rematch` decides a request: completed and rated, within 2:00 of the end, both players with a fresh tab on the match (`presence_tabs.match_id`, hidden counts), one per match; a crossed press accepts. A request lasts 30s (`expires_at`, expired by the sweep and lazily); a decline or expiry starts the pair's 60s cooldown shared with challenges (`pair_cooldown_until`), outside the three-declines rule. The caller's offer (`readRematchOffer`, never in the broadcast state) reaches the client on the state route; the view is `deriveRematchView`. With the slip lifted or in review, a request is the ledger's first line with the cue, the `(1)` title and the favicon; a rematch and a third party's call share the state row, rematch first. The new table replaces the result in history; its scoreboard names the series on the clock row (`match 2 · Birna 1–0`). The functions are called only from `lib/match/rematchService.ts` (`accept_rematch` stays in `createMatch.ts`).
- **Review.** `?review=n` on the match page, the same field (`useMatchReview`): steps are every received move in `global_seq` order from the public, cacheable `GET /api/match/[matchId]/moves` (`buildReviewSteps`; refusals are steps; a closing step applies unplayed penalties so the last step equals the result). The scoreboard's clock row is the scrubber (a slider owning ←/→, Home/End, Space); the ledger's cells jump to their steps (one grid, roving tabindex); the step's row carries the cursor line; `first · back · play ▸ · next · last` (phone: glyphs in the pinned foot). The URL is written with the native History API carrying only `{ kind }` (a state with the router's `__NA` marker is not synced into `useSearchParams`). Entering review adds one entry; Back walks review → result → lobby. `/summary` → `?review=last`. Anyone may read a finished match, signed out included (`matchPageAccess`; the transport's `reader` mode polls and subscribes to nothing).
One migration, `supabase/migrations/20260926001_result_rematch_review.sql`. Tasks: `specs/071-result-rematch-review/tasks.md`.

**Invite links and profiles (spec `specs/072-invite-links-profiles/spec.md`, 2026-09-24, branch `072-invite-links-profiles`)**: bring a friend in, and read a player.
- **Invite links.** `invite a friend ▸` (the primary of an empty lobby) makes a single-use, 10-minute link and copies it (`match_links`: only `sha256(token)` is stored; the sender's browser keeps the text in `localStorage` for `copy again ▸`). A link is the sender's one outgoing challenge: every withdraw path cancels it (`send_challenge`, `create_match_between`, sign-out, lobby switch, a search's `withdrawOutgoing`). It stands in the line slot as a `link` state after `sent` and before `search`.
- **Opening a link only reads it** (`read_link` is `stable`), so a chat app's preview cannot use it. `/c/:token` (`/en/c/:token`) signed out is the invite door (the door with the call band, `accept ▸`); signed in it redirects to `/?invite=<token>`, where the link is a call in the slot held by the tab (`LinkCall`, never on `/api/standing`); the sender's own link reads `this is your link · copy ▸`; a live match redirects to it; the other locale redirects to the link's own. Expired, used, cancelled and unknown links read alike. No-store, no-referrer, noindex.
- **Accept is a POST** (`acceptLinkAction`, which signs a visitor in through `lib/auth/signIn.ts` first) → `accept_link`: players then the link locked, compare-and-set, `create_match_between(…, 'link', …)` in `lib/match/createMatch.ts`; the table's deadline becomes the link's expiry (`the table waits · 9:12`). A friend who leaves before the sender sits voids it `not_seated` against the sender, with no table-leave cooldown (`void_table`). The sender goes to a used link's table (`OutgoingLink.matchId`). 100 rounds of two concurrent accepts make one match each (`tests/integration/db/link.race.test.ts`).
- **Profiles.** `readProfile` returns a `ProfileView` that never carries a last-seen time, a status or an avatar (a key allow-list test pins it): the rating in the page's language, peak, the week's change, a 30-day chart, the last ten, the record (win rate = won ÷ matches), three best words (`best_words`, distinct, void and abandoned excluded) as word strips, and recent matches (own) or your matches against them (another's). Another player's profile shows one presence word (`presence_word`, or the live lobby row) and `challenge ▸` with the viewer's stakes as its one primary, opening the lobby's composer (`ComposerPanel`) in column B. Your own handle redirects to `/profile`; an unknown one is a 404. The old `getPlayerProfile*`, `getBestWords`, `ProfilePage` and `PlayerProfile` are gone, with the room's `profile` fixture.
- **Rules.** In the page frame; the primary by state (`rulesPrimary`: `close this tab ▸` from a match's `⋯` via `?from=`, `find an opponent ▸`, `enter the lobby ▸`); `how to play ▸` in every masthead (marked current on `/rules`) and the phone `⋯`; the clock line no longer says it blinks. The review `⋯` copies the review's link (`copy link ▸`).
One migration, `supabase/migrations/20260927001_invite_links_profiles.sql`. The link functions are called only from `lib/matchmaking/linkService.ts` (`accept_link` from `createMatch.ts`), `best_words` and `presence_word` only from `lib/profile/profileRepository.ts`. Tasks: `specs/072-invite-links-profiles/tasks.md`.
...
Fixture phases (45): table, table-seated, void, void-queue, table-link-waits (spec 072), idle, picking, illegal, reveal, final, disconnect, phone-sheet, resign, over-slip, rules, scoring, scored, opp-reveal, rejected, done-waiting, time-up, end-early, low-clock, last-seconds, starting, missed, stakes, pick-cleared, last-moved, gone, offline, leave (spec 070; the sign-in, lobby, queue and paused-search phases went with their rooms), and spec 071's result-moves, result-incomplete, result-both, result-forfeit, result-early, rematch-sent, rematch-in, rematch-declined, review, review-refused, review-time, review-public, rematch-in-review. Pages have their own fixtures at `/dev/page?phase=…` (door, is-door, door-returning, lobby, is-lobby, lobby-new, lobby-empty, composer, is-composer, the slot states, and spec 072's lobby-link-out, is-lobby-link-out, lobby-link-refused, invite-door, is-invite-door, invite-door-expired, invite-door-returning, lobby-link-call, is-lobby-link-call, lobby-own-link, profile-own, is-profile-own, profile-own-new, profile-own-call, profile-public, is-profile-public, profile-public-sent, profile-public-in-match, profile-public-away, profile-public-signed-out). The phone views `phone-match`, `phone-match-664` and `phone-match-360` (over `idle`) and `phone-table`, `phone-table-664` and `phone-table-360` (over `table`) are viewport tests with their own baselines, not phases.
...
pnpm perf:link-accept         # Assert create/accept an invite link <200ms, read <50ms p95, live local Supabase (spec 072)
...
- `/lib` - Business logic and utilities
  ...
  - `/lib/profile` - Profiles (spec 072): `readProfile` (the one server read, `ProfileView`), `profileRepository` (`best_words`, `presence_word`), `readHandle` (`profileHandlePath`), and the pure `profileHeader`, `record`, `weekChange`, `chartSeries`, `bestWords`, `presenceLine`, `publicPrimary`
...
- `/components` - React Client Components:
  ...
  - `/components/profile` — `ProfileOwnPage`, `ProfilePublicPage`, `ProfileParts` (header, record row, word strip, matches), `ProfileChart` (spec 072)
...
6. **Spec 071 Icelandic strings** marked `// native-read` in `lib/i18n/copy/is.ts`: `Kári hætti að spila` (the early end's detail, name-safe for `farinn`), `Kári hætti við`, `Kári hóf aðra viðureign`, `Kári fór` / `fór` (has left), `hafnað · frosinn` / `færður`, `−15 óleikið`, `ekki komið að`, `skref yfirferðar`. The owner's `yfirfara viðureignina ▸`, `annan leik? ▸` and `hvorugt kláraði` are in.
7. **Spec 070 Icelandic strings** need the same native read: the door and lobby copy in `lib/i18n/copy/pages.is.ts` (lines marked `// native-read`), and the name-safe rewrites of 2026-09-24 (`<nafn> · beðið svars`, `mótspilari · <nafn>`, `aftur í sambandi · 0:34 án tengingar`, `sá leikmaður er ekki lengur hér`).
8. **Identity is a bridge (spec 067).** Losing a browser's cookies loses its names until Supabase Auth (next phase) brings recovery. The Icelandic `sign_out_in_match` (`kláraðu viðureignina fyrst`) and returning-door strings need the native read in gap 4. `WOTTLE_SESSION_SECRET` must be set in Vercel before release, and the release signs everyone out once (no grace for unsigned cookies).

9. **Spec 072 Icelandic strings** marked `// native-read` in `lib/i18n/copy/pages.is.ts` and `is.ts`: `Tengill úti`, `Tengill tilbúinn`, `nýr tengill ▸`, `ógilda tengil ▸`, `tengill ógiltur` / `tengillinn rann út`, `Hekla býður þér með tengli`, `tengillinn þinn fellur úr gildi`, `tengill úti · 9:58` (title), `enginn tengill búinn til · reyndu aftur`, `bara inn í lobbíið`, the invite door's consequence (`Ef þú samþykkir … sest við borðið · Kári`, name-safe), `borðið bíður · 9:12`, `fjarverandi`, `í enska lobbíinu` / `í íslenska lobbíinu`, and the rules' new clock line (`dökknar hún og síðustu 15 sekúndurnar eru taldar í orðum; ekkert blikkar`, `í stigatöflunni fyrir ofan borðið`).
...
- TypeScript 5.x, Node.js 22, React 19, Next.js 16.2 (App Router, `proxy.ts`); PL/pgSQL. + Supabase JS v2 (RPC; Realtime pokes from spec 070), Zod, zustand, Tailwind 4, Node `crypto` (`randomBytes`, `createHash`). Browser: the Clipboard API (with a selectable-text fallback), Web Share where available, `localStorage` (the sender's link text, R2), `window.close()`. No new dependency. (072-invite-links-profiles)
- Supabase PostgreSQL. One additive migration, `20260927001_invite_links_profiles.sql`: `match_links` (hashed token, single use), 7 new functions (`create_link`, `read_link`, `accept_link`, `cancel_link`, `expire_links`, `best_words`, `presence_word`), 6 changed. (072-invite-links-profiles)
