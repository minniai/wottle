...
- `/app` - Next.js App Router
  ...
  - `/app/[locale]/(room)` - The **room** route group: `layout.tsx` reads the session once → `RoomShell`; pages `/` (landing = lobby with an empty seat), `/lobby`, `/matchmaking` (queue → found → match in place), `/match/[matchId]` (non-participant guard: live → `/lobby`, completed → read-only)
...
4. **Icelandic translation review** — `lib/i18n/copy/is.ts` and `components/rules/content/is.tsx` were drafted in the implementation and need a native speaker's read before release.
