import { BOARD_SIZE, BOARD_TILE_COUNT } from "@/lib/constants/board";
import type { Coordinate } from "@/lib/types/board";

/**
 * Deterministic board generation. Pure and browser-safe: the same seed always
 * yields the same grid (cyrb128 hash → sfc32 PRNG), every alphabet letter
 * appears at least once, the rest is weighted by letter frequency. Used
 * server-side for `matches.board_seed` and client-side for the lobby warm-up
 * field and the matchmaking placeholder (spec 044, research R3).
 */
...
/** Coordinates whose letter differs between two boards (row-major order). */
export function diffBoards(a: string[][], b: string[][]): Coordinate[] {
  const diffs: Coordinate[] = [];
  for (let y = 0; y < a.length; y += 1) {
    for (let x = 0; x < a[y].length; x += 1) {
      if (a[y][x] !== b[y]?.[x]) diffs.push({ x, y });
    }
  }
  return diffs;
}
