...
// forked from design-sync lib/dts.mjs — Wottle has no dist .d.ts tree; parse the
// component .tsx sources (with the @/* alias) so <Name>Props interfaces resolve.
...
function projectFor(pkgDir, typesRoot) {
  ...
  const project = new Project({
    ...
    compilerOptions: {
      ...
      skipLibCheck: true,
      strict: false,
      // Fork: resolve the app's "@/…" alias so component sources typecheck.
      baseUrl: pkgDir,
      paths: { '@/*': [join(pkgDir, '*')] },
    },
  });
  ...
  // Fork: no shipped .d.ts tree exists — add the component sources and shared
  // type modules so <Name>Props interfaces and their imports resolve.
  project.addSourceFilesAtPaths([
    `${pkgDir}/components/**/*.tsx`,
    `${pkgDir}/lib/**/*.ts`,
    `!${pkgDir}/**/node_modules/**`,
    `!${pkgDir}/**/*.spec.*`,
    `!${pkgDir}/**/*.test.*`,
  ]);
  console.error(`  [DTS] parsed ${project.getSourceFiles().length} .d.ts/.tsx files from ${root} + src (fork)`);
