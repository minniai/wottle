...
- [ ] T001 Create `GameConfig` and `MoveEvaluation` TypeScript interfaces in `lib/types/index.ts` (or appropriate types file)
- [ ] T002 Create `DEFAULT_GAME_CONFIG` constant in `lib/constants/game-config.ts`
...
- [ ] T003 [US1] Remove hardcoded board size and limits in `lib/game-engine/board.ts` and replace with `GameConfig` parameters
- [ ] T004 [US1] Update `app/actions/game.ts` (or equivalent orchestrator) to instantiate/use `DEFAULT_GAME_CONFIG`
...
- [ ] T005 [P] [US2] Write unit tests in `tests/unit/game-engine/word-finder.test.ts` for orthogonal adjacency validation (Red phase)
- [ ] T006 [P] [US2] Write unit tests in `tests/unit/game-engine/scorer.test.ts` for scoring multiple crossing words without diagonals (Red phase)
- [ ] T007 [US2] Implement purely orthogonal (non-diagonal) extraction logic in `lib/game-engine/word-finder.ts` (Green phase)
- [ ] T008 [US2] Implement recursive cross-word adjacency scanning in `lib/game-engine/word-finder.ts` (Green phase)
- [ ] T009 [US2] Update `lib/game-engine/validator.ts` (or equivalent) to iterate through all extracted `words` from the move and validate each against the Trie dictionary
- [ ] T010 [US2] Update `lib/game-engine/scorer.ts` to sum the scores of all uniquely formed valid cross-words, explicitly ignoring diagonal sequences

## Phase 4: Polish & Integration

_Goal: Ensure the overarching game loop respects the new validation returns and that all existing tests pass._

- [ ] T011 Update integration tests in `tests/integration/` to reflect the new strict orthogonal validation rules
- [ ] T012 Run performance benchmarks (if any exist for engine validation) to ensure the recursive cross-word logic executes under the 50ms SLA
