...
**Regression tests to pin (P0)**: `FÁR/RÁF scores twice; a single-direction run produces one record`; `BORÐA + GILT is rejected` (dictionary has `borða` and `gilt` but not `borðagilt`); `deriveReadingDirection` for all four vectors and a 1-tile degenerate input.
