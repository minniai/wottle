...
├── unit/styles/{tokens,acceptance-grep}.test.ts          # replace the eight previous-theme regex tests
...
| **P1** | tokens/fonts/Tailwind; `room.css` skeleton; `(room)` layout + `RoomShell` + `roomStore` (match phase only, wrapping existing `MatchClient` internals temporarily); `PlayerBar` + `ClockLane` replacing the stat cards and compact bars; TopBar removed; `RoomMenu` | — | `HudCard`, `PlayerPanel`, `TimerDisplay`, `PlayerAvatar`, `MatchCenterChrome`, `RoundPipBar`, `TopBar`, `UserMenu`, `deriveClockUrgency` tones | style tests rewritten; `room-layout.spec` at three viewports; two-player flow green |
