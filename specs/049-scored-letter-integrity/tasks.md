...
- [x] T026 [US2] [impl] `lib/room/bandGeometry.ts`: `ownerSeatOf`, `wordCells` on `WordBand`, `bandCells` filters by owner, `chevronPath` placed from `wordCells`, `sharedCells` and `seatOfCell` removed
...
- [x] T028 [US2] [impl] `components/room/Field.tsx` (seat from `ownerSeatOf`; `shared` derivation removed; highlight from `wordCells`), `components/room/FieldCell.tsx` (`CellState` −`"shared"`), `components/room/FieldBands.tsx` (draw over `cells`, chevron from `wordCells`, dim by `wordCells`)
- [x] T029 [P] [US2] [test] `tests/unit/styles/room-css.test.ts` (extend): no `[data-state="shared"]` rule remains; `tests/unit/styles/acceptance-grep.test.ts`: `"shared"` as a cell state and `sharedCells` return nothing under `components/` and `app/styles/`
