...
- **Visual baselines:** a first run against a dev server started before the branch switch wrote the retired opponent colour into the new baselines; they were deleted and regenerated on a fresh server, and `starting` and `end-early` were regenerated from scratch because their changes fell under the diff tolerance.
