...
- **Visual baselines:** a first run against a dev server started before the branch switch wrote the opponent seat colour into the new baselines; they were deleted and regenerated on a fresh server, and `starting` and `end-early` were regenerated from scratch because their changes fell under the diff tolerance.
