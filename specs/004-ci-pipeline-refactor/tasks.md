...
- [ ] T008 [P] [US2] In the `perf-gate` job in `.github/workflows/ci.yml`, insert the identical Supabase Docker layer caching sequence (same 4-step pattern as T007) after `supabase/setup-cli@v1` and before `pnpm quickstart` (FR-006)
- [ ] T009 [P] [US2] In the `quickstart` job in `.github/workflows/ci.yml`, insert the identical Supabase Docker layer caching sequence (same 4-step pattern as T007) after `supabase/setup-cli@v1` and before `pnpm quickstart` (FR-006)
...
- [ ] T012 [P] [US2] In the `perf-gate` job in `.github/workflows/ci.yml`: (a) change `needs:` from `[playwright]` to `[build]`, (b) remove the `pnpm build` step, (c) add `actions/download-artifact@v4` step for `next-build-${{ github.run_id }}-${{ github.run_attempt }}` to `path: .next/`, (d) add BUILD_ID integrity check step matching T011 (FR-008)
