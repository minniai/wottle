...
6. **Failure mode (FR-011)** — if `instantScoreFirstSubmission` throws or exceeds a 500 ms internal budget, log `instant-scoring.failed`, do not publish a partial reveal, and let `advanceRound` (already queued in the same `after()` hook) do the combined path. The fast-path never leaves clients in a half-revealed state because failure means we simply don't publish.
