...
const NO_STANDING = {
  ...
  outgoing: false,
  callUp: false,
  overlays: new Map<string, RowOverlay>(),
};
...
function InviteBelow({
  ...
  return (
    ...
    >
      <button
        type="button"
        className="page-link page-link--ink"
        onClick={onInvite}
...
export function Lobby({
  ...
  const plan = onInvite
    ? lobbyPrimary({ othersHere: rows.length, find: pageModel, copy })
    : { invite: "hidden" as const, find: pageModel, inviteNote: null };
  const model = plan.find;
  const invitePrimary =
    plan.invite === "primary" && onInvite ? (
      <button
        type="button"
        className="lobby-block__find action-primary page-primary"
        onClick={onInvite}
        data-testid="lobby-invite"
      >
        {copy.pages.INVITE_A_FRIEND}
  ...
  const findButton =
    model.find === "hidden" ? null : (
      <button
        type="button"
        className={`lobby-block__find ${model.find === "secondary" ? "page-link page-link--ink" : "action-primary page-primary"}`}
        onClick={onFind}
        data-testid="lobby-find"
      >
        {copy.FIND_OPPONENT}
      </button>
    );
    ...
    <>
      ...
      {empty && arrival ? (
        <button
          type="button"
          className="page-link page-link--ink"
          onClick={arrival.armed ? arrival.cancel : arrival.arm}
          data-testid="lobby-tell-me"
        >
    ...
    >
      <div className="page-col-a">
        ...
        {plan.invite !== "primary" && onInvite && !empty ? (
          <InviteBelow
            onInvite={onInvite}
            note={plan.inviteNote ?? copy.pages.linkWorksFor(LINK_TTL_MS / 60_000)}
            hidden={plan.invite === "hidden"}
          />
        ) : null}
      </div>
