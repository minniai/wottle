...
/** Rows of name, score (yours in your colour), result and review. Never a void or abandoned match. */
export function RecentRows({ games }: { games: RecentGameRow[] }) {
  const copy = useCopy();
...
/** Your last matches with no last match to draw (game flow B1): four rows under their own heading. */
export function RecentMatches({ recent }: { recent: RecentGameRow[] }) {
  ...
  return (
    <section className="recent" aria-labelledby="recent-label">
      ...
      <RecentRows games={recent.slice(0, SHOWN)} />
    </section>
  );
}
