 ...
 */
/**
 * `reader` (spec 071 FR-042): someone reading a finished match they did not play. Nothing
 * changes, so nothing is subscribed or polled, and no presence of theirs reaches the channel.
 */
export type TransportMode = "participant" | "reader";

export function useMatchTransport(
  matchId: string,
  currentPlayerId: string,
  pollIntervalMs = 3_000,
  mode: TransportMode = "participant",
): TransportState {
  ...
  }, [matchId, currentPlayerId, usePolling, reading, applySnapshot, applyResolution, fallBack]);
  ...
  }, [matchId, usePolling, reading, pollIntervalMs, applySnapshot, attention]);
  ...
  }, [matchId, reading, applySnapshot, onPoll, attention]);
